...
class LongformerSelfAttention(nn.Module):
    ...
    def sliding_window_attention(self, queries, keys, values, window_size):
        B, query_len, heads, head_dim = queries.shape
        key_len = keys.shape[1]

        # Initialize a tensor to store the attention outputs
        local_attention_output = torch.zeros(B, query_len, heads, head_dim).to(queries.device)

        for i in range(0, query_len):
            # Define the window boundaries
            window_start = max(i - window_size, 0)
            window_end = i + 1  # +1 as the end index is exclusive

            # Slice the keys, values and queries according to the window
            keys_window = keys[:, window_start:window_end, :, :]
            values_window = values[:, window_start:window_end, :, :]
            queries_window = queries[:, i:i + 1, :, :]  # +1 to keep the dimension

            # Calculate energy (Scaled Dot-Product)
            energy_window = torch.einsum("bqhd,bkhd->bhqk", [queries_window, keys_window])

            # Calculate attention scores
            attention_window = torch.softmax(energy_window / (self.embed_size ** (1 / 2)), dim=3)

            # Calculate the attention output
            out_window = torch.einsum("bhql,blhd->bqhd", [attention_window, values_window])

            # Store the result in the corresponding position of the output tensor
            local_attention_output[:, i:i + 1, :, :] = out_window
    ...
    def forward(self, query, keys, values, mask):
        ...
        # Local attention
        local_attention_output = self.sliding_window_attention(queries, keys, values, self.window_size)

        # Global attention logic (assuming first token is the global token)
        global_queries = queries[:, 0:1, :, :]
        global_keys = keys[:, 0:1, :, :]
        global_values = values[:, 0:1, :, :]
        ...
        out = local_attention_output + global_out
...
class LongformerDecoderBlock(nn.Module):
    def __init__(self, embed_size, heads, forward_expansion, dropout, device, window_size):
        super(LongformerDecoderBlock, self).__init__()
        self.longformer_attention1 = LongformerSelfAttention(embed_size, heads, window_size)
        self.longformer_attention2 = LongformerSelfAttention(embed_size, heads, window_size)
    ...
    def forward(self, x, value, key, src_mask, trg_mask):
        # First Longformer attention
        attention1 = self.longformer_attention1(x, x, x, trg_mask)
        query1 = self.dropout(self.norm1(attention1 + x))
        
        # Second Longformer attention
        attention2 = self.longformer_attention2(query1, key, value, src_mask)
        query2 = self.dropout(self.norm2(attention2 + query1))

        # Feed-forward network
        out = self.feed_forward(query2)
        return out
...
class Pose2AudioLongformer(nn.Module):
    ...
    def make_longformer_trg_mask(self, trg, window_size):
        """
        Create a mask for Longformer decoder's target.

        Args:
            trg (Tensor): target sequences [batch_size, seq_len]
            window_size (int): size of the attention window

        Returns:
            trg_mask (Tensor): The mask for the target tokens [batch_size, 1, seq_len, seq_len]
        """
        B, seq_length = trg.shape
        trg_mask = torch.zeros((B, seq_length, seq_length)).to(trg.device)

        # Fill in the lower triangular part for autoregressive attention
        for i in range(seq_length):
            window_start = max(i - window_size, 0)
            window_end = i + 1  # +1 because the end index is exclusive
            trg_mask[:, i, window_start:window_end] = 1

        # Enable global attention on the first token
        trg_mask[:, :, 0] = 1

        # Expand dimensions to match the attention mask shape
    ...
    def forward(self, src, trg, src_mask = None):
        ...
        trg_mask = self.make_longformer_trg_mask(trg, self.window_size)

...
def train():
    ...
    for epoch in range(num_epochs):
        ...
        for i, (audio_codes, pose, pose_mask, wav, wav_mask, wav_path, vid_path, _) in progress_bar:    
            ...
            gradient_penalty = compute_gradient_penalty(discriminator, real_data.data, fake_data.data, device)
            ...
            writer.add_scalar(f'Loss_Discriminator_lr={str(args.d_learning_rate)}', loss_d.item(), epoch * len(train_loader) + i)
            writer.add_scalar(f'Loss_Discriminator_lr={str(args.d_learning_rate)}/Real', loss_d_real.item(), epoch * len(train_loader) + i)
            writer.add_scalar(f'Loss_Discriminator_lr={str(args.d_learning_rate)}/Fake', loss_d_fake.item(), epoch * len(train_loader) + i)
            writer.add_scalar(f'Loss_Discriminator_lr={str(args.d_learning_rate)}/Gradient_Penalty', loss_gp.item(), epoch * len(train_loader) + i)
            ...
            if (i + 1) % N_CRITIC == 0:
                ...
                if len(rolling_mel_mse_loss) < 10:
                ...
                # scaled_batch_nll_loss = batch_nll_loss / (sum(rolling_batch_nll_loss)/len(rolling_batch_nll_loss))

                combined_loss_g = (2*batch_nll_loss) + adversarial_loss_g + scaled_mel_mse_loss
                ...
                writer.add_scalar(f'Loss_Generator_lr={str(args.g_learning_rate)}/1_Combined_Loss', combined_loss_g.item(), epoch * len(train_loader) + i)
                writer.add_scalar(f'Loss_Generator_lr={str(args.g_learning_rate)}/2_Batch_NLL_Loss', 2*batch_nll_loss.item(), epoch * len(train_loader) + i)
                writer.add_scalar(f'Loss_Generator_lr={str(args.g_learning_rate)}/3_Mel_MSE_Loss_Scaled', scaled_mel_mse_loss.item(), epoch * len(train_loader) + i)
                writer.add_scalar(f'Loss_Generator_lr={str(args.g_learning_rate)}/4_Adversarial_Loss', adversarial_loss_g.item(), epoch * len(train_loader) + i)
                writer.add_scalar(f'Loss_Generator_lr={str(args.g_learning_rate)}/5_Average_Epcoch_Loss', avg_epoch_loss_g, epoch * len(train_loader) + i)

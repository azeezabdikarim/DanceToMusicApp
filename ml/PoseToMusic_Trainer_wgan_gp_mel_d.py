...
def train():
    ...
    discriminator = AudioCodeDiscriminator(target_shape, 
                                       ...
                                       sigmoid_out = False)
    discriminator = MelSpectrogramDiscriminator(input_shape=(1, 64, None), 
                                            hidden_units=discriminator_hidden_units, 
                                            num_hidden_layers=discriminator_num_hidden_layers, 
                                            alpha=discriminator_alpha, 
                                            sigmoid_out=False)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (audio_codes, pose, pose_mask, wav, wav_mask, wav_path, vid_path, _) in progress_bar:    
            ...
            if (i + 1) % N_CRITIC == 0:
                ...
                writer.add_scalar('Loss_Generator/2_Batch_NLL_Loss', 2*batch_nll_loss.item(), epoch * len(train_loader) + i)

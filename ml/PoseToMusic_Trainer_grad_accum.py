...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for i, (audio_codes, pose, pose_mask, wav, wav_mask, wav_path, sample_rate) in progress_bar:            
            ...
            loss_g_fool_disc = criterion_d(output, real_labels) # Train to fool the discriminator
            avg_nll_loss = total_nll_loss / (target.shape[1] - 1)   
            loss_g = loss_g_fool_disc + avg_nll_loss + mel_mse_loss
            ...
            if (i + 1) % accumulation_steps == 0 :
                ...
                writer.add_scalar('Loss/Generator/Total', loss_g.item(), epoch * len(train_loader) + i)
                writer.add_scalar('Loss/Generator/Fool Discriminator', loss_g_fool_disc.item(), epoch * len(train_loader) + i)
                writer.add_scalar('Loss/Generator/Average NLL', avg_nll_loss.item(), epoch * len(train_loader) + i)
                writer.add_scalar('Loss/Generator/Mel Loss', mel_mse_loss, epoch * len(train_loader) + i)
                writer.add_scalar('Loss/Discriminator/Total', loss_d.item(), epoch * len(train_loader) + i)
        ...
        if total_loss_g < best_loss:
                        ...
                        g_last_saved_model = save_model(pose_model, model_save_dir, best_loss, g_last_saved_model, name='gen_5_sec_dnb_')
                        d_last_saved_model = save_model(descriminator, model_save_dir, best_loss, d_last_saved_model, name='disc_5_sec_dnb_')
                        encodec_last_saved_model = save_model(encodec_model, model_save_dir, best_loss, encodec_last_saved_model, name='encodec_5_sec_dnb_')

...
from .bod_music_processor import *
...
class convNet(nn.Module):
    ...
    def torch_infer(self, feats, device, minibatch=1):
        with torch.no_grad():
            inference = []
            for x in self.torch_infer_data_builder(feats, minibatch=minibatch):
                x = x.to(device)
                output = self(x, minibatch=x.shape[0])
                inference.append(output.cpu())

            # Use torch.cat to concatenate all tensors along the first dimension and then flatten
            return torch.cat(inference).flatten()

    def torch_infer_data_builder(self, feats, soundlen=15, minibatch=1):
        x = []
        for i in range(feats.shape[2] - soundlen):
            x.append(feats[:, :, i:i + soundlen])

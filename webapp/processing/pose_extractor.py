...
def saveVideoKeypoints(video_path, pose_model, frame_rate = 24, mp_pose=None):
    ...
    color_vid_save_path = f"{video_name}_3D_render.mp4"
    normalized_vid_save_path = f"{video_name}_3D_normalized_render.mp4"
    ...
    keypoint_render_path = f"{video_name}_points_and_audio.mp4"
    video.write_videofile(keypoint_render_path)

    video_normalized = VideoFileClip(normalized_vid_save_path)
    video_normalized = video_normalized.set_audio(audio)
    video_normalized_save_path = f"{video_name}_normalized_with_audio.mp4"
    video_normalized.write_videofile(video_normalized_save_path)


    # Convert list to a NumPy array of shape [num_frames, num_keypoints, 3]
    final_landmarks_array = np.stack(landmarks_list, axis=0)
    pose_save_path = f"{video_name}_3D_landmarks.npy"
    ...
    return keypoint_render_path, pose_save_path

def extractPoses(video_path):
    mp_pose = mp.solutions.pose
    pose = mp_pose.Pose(static_image_mode=False, model_complexity=2, enable_segmentation=False, smooth_landmarks=True)

    print("extract Poses: ", video_path)
    output_dir = saveVideoKeypoints(video_path, pose, mp_pose=mp_pose)
    return output_dir
